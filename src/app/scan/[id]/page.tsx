...
import { Building2, ShieldCheck, Tag, Info, PhoneCall, Cpu, Layers, Calendar, UserCheck, Loader2 } from "lucide-react";
...
export default function PublicScanPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then((p) => {
      setId(p.id);
      fetch(`/api/scan/${encodeURIComponent(p.id)}`)
        .then(async (res) => {
          if (res.status === 404) {
            setNotFound(true);
            return;
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-md space-y-6 text-center">
          ...
          <div className="flex flex-col items-center gap-3 py-10">
            ...
            <p className="text-sm text-muted-foreground">Verifying asset tag...</p>
          </div>
        </div>
      </div>
    );
          ...
          <Card className="shadow-lg border-muted text-center p-6 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                The scanned identifier <span className="font-mono font-semibold text-foreground">{decodeURIComponent(id)}</span> does not match an active record in the system.
              </p>
            </div>
